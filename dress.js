const dressPage = document.querySelector(".products")
console.log(dressPage)
    
class Products {
    async getProducts() {
      try {
      let result = await fetch('http://localhost:3000/dress.json'); 
      let data = await result.json();
      console.log(data)
      let products = data.dress;
      return products
    } catch(error){
      console.log(error)
    }
