const artsPage = document.querySelector(".products")
console.log(artsPage)
    
class Products {
    async getProducts() {
      try {
      let result = await fetch('http://localhost:3000/arts.json'); 
      let data = await result.json();
      console.log(data)
      let products = data.arts;
      return products
    } catch(error){
      console.log(error)
    }
